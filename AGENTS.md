...
> Owner directive, August 5, 2026, binding on every agent (`claude-*`, `codex-*`, `gemma`, and all future agents). Codified August 6, 2026 because the rule was binding and cited by number — including by ADR-061 — while appearing **nowhere in this repository**, so no agent could look it up, quote it, or check its precedence against the rules it contradicts.
>
...
*   **Refs**: ADR-061 (enforcement); A27 §2.24 (heartbeat loop, extended here); Rule 17 (overridden here); A24 (the other Rule 17 override); A32 (owner reporting standard).
