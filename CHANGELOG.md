...
- **The menu-bar icon is now a branded mark, not a bare colored dot** (claude-pantheon, 2026-06-17). The status item set `button.title` to a text string that `titleLabel()` overwrote with a traffic-light **emoji** (`🟢`/`🟡`/`🔴`) — so the entire product showed in the macOS menu bar as a generic colored dot, unbranded and indistinguishable from any other app (and the fallback hieroglyph `𓁢` would have been a tofu box on any Mac whose menu-bar font lacks Egyptian Hieroglyphs). Replaced with the **Eye (Horus — the watchful protector) as an SF Symbol `eye.fill` template image**: vector, guaranteed to render on macOS ≥13, adapts to menu-bar height + light/dark, and **tinted by the health band** via `contentTintColor` — native monochrome when healthy (the icon stays quiet), amber/red only when something needs you. The reclaimable-waste figure (≥1 GB) rides beside the Eye. `swift build` green. Refs: ADR-030, the green/amber/red rubric (#50/#51).
