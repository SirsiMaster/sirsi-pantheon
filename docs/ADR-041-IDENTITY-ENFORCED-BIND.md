...
4. **App-token events re-trigger workflows** — unlike `secrets.GITHUB_TOKEN` events. That asymmetry is precisely why a review works where #217's bot label did not. The gate is nonetheless **deliberately NOT triggered by `pull_request_review`**: such a run resolves `GITHUB_SHA` against the *base* branch, so its check lands on the wrong commit and the PR's required check never updates. `scripts/bind/sirsi-bind.sh` re-runs the PR's own `pull_request` run after approving — correct PR context, correct head SHA — and the gate re-reads reviews from the API. Do not "fix" this by adding the trigger; verify the event mechanics first, because assuming them is what shipped #217.
...
6. **Scope — authority-model paths only:** `.github/`, `scripts/bind/`, `cmd/sirsi/`, `internal/router/`, `PANTHEON_RULES.md`, `docs/ADR-*`. **`scripts/bind/` is in scope because it holds the approver itself** — a gate that does not cover the mechanism clearing it is not a gate: a PR editing only `sirsi-bind.sh` would otherwise change what a bind means, or where the key is read from, with no bind of its own. Found by codex-pantheon SME validation, 2026-07-15. Everything else merges autonomously, unchanged. The explicit opt-in `binding-hold` label is untouched and still holds any PR on request.
