...
**Deliberately NOT yet done — the cutover.** Files are still written (audit view) and still tracked in git. Stopping file writes and untracking `items/` ends the audit-view-in-git era and changes what every legacy tool sees; that is an owner-visible decision at the end of the deprecation window, not a side effect of a build PR. Until then the file view stays complete and byte-faithful.
