...
| C3b | M5 `~/.local/bin/sirsi` | sha256 `0b2fd3da2889b33a…` — built 2026-09-13 08:39Z from clean worktree `~/.sirsi/worktrees/main-3d535ebc` detached at C1 `1995046` (PR #755), `go build -trimpath` → `.new` → atomic `mv`; previous `59688f20…` retained as `~/.sirsi/candidates/sirsi-pre-755` (and `sirsi-pre-3d535ebc`); the `2ea58a1c…` build stays withdrawn | *reconstructed* C1→C3b: lineage Ra-reported (no `vcs.revision` under trimpath; D5 open) | hash verified; **live-verified** 2026-09-13 08:41–42Z — relay `system/ai.sirsi.router.relay` pid 71606 (`_sirsipantheon`) forwards `status=200`, all 9 running wake loops `running` exit 0 with `runs` not climbing 84 s after kickstart, `spool://` round-trip 4.8 s from the new binary. See Change Log 2026-09-13 (fix). |
...
- 2026-09-15 (ops) — **M5 relay daemon env repaired: `SIRSI_RELAY_TRUST_GROUP=_sirsipantheon` added to `/Library/LaunchDaemons/ai.sirsi.router.relay.plist`** (backup `/var/sirsipantheon/relay-plist.bak-20260915`). Symptom (SHA `20260914-234610`): `ctr`/`node-status`/`doctor` returned nothing within 10 s. Two causes, both measured live: (1) the daemon ran WITHOUT the trust group — responses were written `0600 _sirsipantheon`, readable by the lane user only inside `Mac/` (an ACL, `+`, on that one dir); lane dirs a client had created itself (`claude-io/`, `M5.local/`, both `0700`) got a forwarded `status=200` and a response the client could never open, so every call there waited the full 30 s (`OUTCOME UNKNOWN`). (2) `node-status` pulled `ListAll` (36 MB) through the relay. Repair: plist edited with `plutil -insert`, then **`bootout` + `bootstrap`** — `kickstart -k` restarts the process with the OLD environment (verified: no "trusting group" note in the start line until the bootstrap); the two lane dirs `chmod 770`; spool root converged to `0770` by `CheckSpoolDirTrustingGroup` at start. Verified: start line carries `(trusting group "_sirsipantheon")`, `SIRSI_AGENT_ID=claude-io` round-trip answers in <10 s, the 9 running lanes untouched (`state = running`, never exited). Code side in C1 `fix/router-stall-gate-spool-fail-fast` (stall gate, fail-fast, owned-dir converge, relay unreadable-lane log, `wake-install` carries the trust group, `node-status` → `Inbox("")`). C3b binary NOT rolled; roll at the next M5 change per §4. Also seen: a 36 MB `ListAll` response file left in `Mac/res` from a timed-out `node-status` (relay sweep reclaims it).
- 2026-09-13 (fix) — **PR #755 merged as C1 `1995046`** (squash of `73bdabf`; CI all five checks green; owner-waived bind, same rollout). `mkdirTrusted` stat-first/chmod-only-what-it-created + apply-schema shape pin 22. **C3b re-rolled** to `0b2fd3da…` from the clean worktree (build → `.new` → atomic `mv`); kickstarted ONLY the nine lanes already in `state = running` (`sudo launchctl asuser 501 launchctl kickstart -k gui/501/ai.sirsi.router.wake.<agent>`) plus the relay, which is a **system** daemon (`sudo launchctl kickstart -k system/ai.sirsi.router.relay`, runs the same `~/.local/bin/sirsi` as `_sirsipantheon` with `--spool /var/sirsipantheon/relay`); the 17 parked lanes were not touched (owner board 2026-08-09). Verified 84 s later: all nine `running`, exit 0, `runs` flat; relay log `/var/sirsipantheon/router-relay.log` shows `relay: forwarded … status=200` at 08:42Z; `sirsi router status ra` over `spool://` answered in 4.8 s. rs-38 fenced-complete. Lesson recorded in the fix's doc comment: a chmod on a path you did not create is a claim of ownership you may not hold; the pre-#754 binary only "worked" because `MkdirAll` on an existing dir is a no-op, so the walk never chmod'd. Noted, out of scope: `~/.sirsi/logs/router-relay.log` (the gui-side log, NOT the daemon's) carries stale `spool: … is a symlink; refusing` lines from a non-daemon invocation path.
