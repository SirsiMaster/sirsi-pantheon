...
## 🪶 Case Study 14: THE CONTEXT TAX — How 4 Open-Source Tools Became 3 Native Packages (2026-04-20)
**Status:** Shipped | **Deities:** RTK, Vault, Horus | **Impact:** 8-49x Context Reduction

### The Problem
AI coding assistants waste 60-90% of context window tokens on noise: ANSI escape codes in terminal output, repeated lines from build tools, full file reads when only function signatures are needed. Four external open-source tools address pieces of this problem — RTK (output filtering), Code Review Graph (AST analysis), Context Mode, and Claude Context (context sandboxing) — but each requires a separate MCP server, a different language (Rust, Python, TypeScript), and external dependencies.

### The Solution
Evaluated all 4 tools. Selected the 3 with real engineering value (RTK, Code Review Graph, Context Mode/Claude Context) and subsumed their core capabilities into native Go packages inside Pantheon. Zero new dependencies.

- **RTK** (`internal/rtk/`) — Filters output at source. Strips ANSI codes, deduplicates repeated lines, truncates oversized output before it reaches the AI context window. 4 files, 12 tests.
- **Vault** (`internal/vault/`) — Sandboxes what's still too large in SQLite FTS5. BM25 code search lets the AI query stored context instead of holding it all in-window. 4 files, 9 tests.
- **Horus** (`internal/horus/`) — Serves structural code graphs (declarations, signatures, type hierarchies) instead of full source files. Go AST-powered symbol extraction. 5 files, 10 tests.

### The Result
16 new files, 31 tests, 10 MCP tools, 8 new Stele event types. Module count: 30 to 33. Deity/module count: 9 to 12.

Horus tested on Pantheon itself: parsed 169 Go files, extracted 328 types, 15 interfaces. File outlines achieve 8-49x reduction (tested: `tools.go` at 700+ lines produces a ~30 line outline = 23x compression).

### Key Insight
The Go standard library (`go/ast`, `go/parser`, `database/sql` + `modernc.org/sqlite`) already has everything needed. No Rust, no Python, no TypeScript, no external MCP servers. The best tools are the ones that don't exist as separate processes.

---

## 𓇶 Case Study 15: RA/HORUS CTR HYPERVISOR — FROM MANUAL SHUTTLE TO AUTONOMOUS RELAY (2026-05)
**Status:** Operational | **Deities:** Ra (Fleet), Horus (Local) | **Impact:** Autonomous Multi-Agent Relay

### The Problem
Building Pantheon required coordinating two AI agents (Claude and Codex) across multiple repositories. The user was the message bus — manually ferrying context, saying "check the router" to move work. State was lost on compaction, and neither agent knew if the other had acted.

### The Solution: Ra/Horus Split (ADR-017)
Ra owns cross-machine orchestration: Idea Router filesystem queue, 17+ registered agents across 6 repos, pluggable executor with writeback verification, autorouter daemon with fsnotify + polling. Horus owns per-desktop visibility: `sirsi router node-status` aggregates daemon health, agent registry, pending work, work-queue statuses, and recent dispatch failures into a single operator view.

### The Result
The autorouter daemon detects state.json changes via fsnotify and dispatches within 200ms. The hardening sprint ran 30+ commits autonomously — 8 review/fix cycles without user message ferrying.
...
### The Problem
The original idea-router was a two-player mailbox hardcoded to `codex` and `claude`. It couldn't address work to Gemini, Qwen, or multiple workstreams. Notification was fire-and-forget with no writeback verification.

### The Solution
Router v3: agent registry, pluggable executor, work item lifecycle tracking, dynamic per-agent inboxes, dispatch ledger with edit-aware re-dispatch. 30+ autonomous handoffs during the hardening sprint.

See: [case-studies/router-v3-multi-agent-queue.md](case-studies/router-v3-multi-agent-queue.md)

---

## Case Study: HARDENING SPRINT — REVERSIBLE CLEANUP (2026-05)
**Status:** Shipped | **Module:** Cleaner, Jackal | **Violations Fixed:** 4

### The Problem
Purge and installer commands called `os.RemoveAll()` directly — bypassing safety checks. On platforms without trash, cleanup silently permanently deleted files.

### The Solution
`DeleteFileReversible()` refuses to permanently delete when no trash is available. `SafetyGateway` centralizes all destructive action confirmation. Every deletion goes through `ValidatePath()` first.

See: [case-studies/hardening-sprint-safety.md](case-studies/hardening-sprint-safety.md)

---

## Case Study: TUI CONTROLLER REFACTOR (2026-05)
**Status:** Shipped | **Module:** Output (TUI) | **Lines:** 2,383 → 322

### The Problem
`tui.go` was a 2,383-line monolith with 6 mutex-guarded global variables. Untestable, unnavigable, structurally capped at 30% coverage.

### The Solution
Split into 14 focused files (max 546 lines). All 6 globals eliminated via message-passing. SafetyGateway interface. 7 controller transition tests.

See: [case-studies/tui-controller-refactor.md](case-studies/tui-controller-refactor.md)
