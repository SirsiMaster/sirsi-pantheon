...
**Status:** PHASES 1–4 COMPLETE (2026-07-27) — Phase 1 built (2026-07-02); Phase 2 contract BOUND (2026-07-04, codex APPROVE — §2b); Phase 2 BUILT (2026-07-04); Phase 3 BUILT (2026-07-05 — one `internal/dispatch` facade wired into CLI + MCP; worker re-arm now gates on the owner-verified end-to-end §2b bar + Phase-4 migration stance); Phase 4 CLOSED 2026-07-27 with live evidence — the engineering preconditions for worker re-arm are met, and the remaining gate is the OWNER'S end-to-end §2b verification, which is deliberately not self-servable (ADR-035; the claude-worker LaunchAgent stays `.quarantined` until the owner clears it)
...
- [x] **Phase 4 — Migration + cutover + back-compat**: three of four deliverables shipped in full; the dual-read window shipped WITHOUT its required `DEPRECATED` warning (recorded below, not retrofitted), verified 2026-07-27 against the live host. `sirsi router migrate` exists and is idempotent and zero-loss — a live run over the real corpus reports `Files seen: 443 / Imported: 1 new, 442 refreshed (count-out 443) / Spot-checked: 3 item(s) byte-compared file↔store / ✔ Zero-loss verified: count-in == count-out`. **Deliverable 2 was NOT fully delivered, and this record now says so.** The requirement was a dual-read window *with a `DEPRECATED` warning*. The union was implemented (#174) and the window genuinely ran and closed — but **no deprecation warning was ever shipped during it**; repository history confirms none existed (codex independent review of #329). Marking the deliverable complete was wrong, and it is exactly the self-assessment that independent review exists to catch: I graded my own work and gave a skipped requirement a pass. The warning is deliberately **not** being retrofitted — the window is closed, the legacy files are frozen, and a deprecation notice for a path nobody can still take would be ceremony rather than information. What is recorded instead is the truth: callers on the legacy path during that window were never warned, and no harm is known to have followed only because #315/#318 subsequently made the store totally authoritative and fail-closed. Mechanics of the window that did ship:  `routercfg.StoreWake()` is live on this host (`~/.sirsi/store-wake` present) and #315 made `Inbox`/`ListAll`/`Show`/`Get` store-only under the cutover while leaving the pre-cutover union byte-identical (Rule 14); #318 made those readers **fail closed**, because degrading to the frozen files on a store-open failure reproduces the very P0 the cutover fixed. **File writes stopped**: #315 retired `internal/liveness`, the last direct file writer, and #318 stopped `SetWake` annotating closed rows; `router migrate`'s importer is now the only writer of the legacy tree, by design, as the recovery path. ADR-036 (`docs/ADR-036-ROUTER-V2-DURABLE-DISPATCH.md`) and ADR-037 (`docs/ADR-037-DAEMON-OWNED-FABRIC.md`) are canon, and A26/A27/A29 carry the registry+wake invariants. Parity at close-out: direct SQL == `router status` == `router dump` == `router pull`. *Done 2026-07-27; checkbox lagged the work — the deliverables landed across #315/#318 rather than one Phase-4 PR, which is why it read open.*
