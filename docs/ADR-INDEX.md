...
**Total ADRs: 51 (+ ADR-031-A/B/C sub-decisions)** | **Next available: ADR-052**
...
| ADR-051 | **Accepted** — Horus as Per-Node Conduit (ADR-028-to-051 renumber). Owner-canon directive: router items + observability = one flow through Horus; one Horus per node; Anubis = SNE + local Horus; Ra = aggregator of Horus ConduitReports. `internal/horus/conduit.go` implements `NodeConduit` + `ConduitReport`. |
