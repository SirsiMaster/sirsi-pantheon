...
| ADR-063 | **Proposed for owner bind** — Ra Charter, completion to completion. Standing mandate over the fabric layer: Ra may move to the next goal only after the current one reaches verified release, and only by a PR that enumerates every step of the next goal before any work starts; fixed owner gates on cost, custody, customer data, cut-over, bind authority; tokens are not progress. |
