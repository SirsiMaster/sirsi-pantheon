...
**Total ADRs: 53 (+ ADR-031-A/B/C sub-decisions)** | **Next available: ADR-055**
...
| [ADR-052](ADR-052-THOTH-FOLD.md) | Fold sirsi-thoth into sirsi-pantheon — Thoth memory system unified under Pantheon as the single source of truth | **Accepted** | 2026-03-30 |
| [ADR-053](ADR-053-THOTH-EXTRACTION.md) | Extract Thoth as Standalone Package — Thoth memory extracted to shared package; sirsi-thoth competitor conflict resolved | **Accepted** | 2026-05-05 |
| [ADR-054](ADR-054-GO-OWNS-THE-SERVING-PATH.md) | Go owns the serving path; Python becomes a called extension — Go is the sole process owner of all Pantheon serving paths; Python/MLX called as subprocess | **Proposed** | 2026-07-27 |
...
| ADR-051 | **Accepted** — Anubis/Ra Product Split: SNE Supervisor Configuration — Pantheon is the sole authority for which SNE profile (`interactive`/`fleet`) each product receives; SNE stays neutral; `configs/supervisor/anubis.yaml` + `ra.yaml` scaffold the binding | **Accepted** | 2026-08-03 |
| ADR-052 | **Accepted** — Fold sirsi-thoth into sirsi-pantheon. Renumbered from ADR-013 (collision with TILED-CONTEXT-RENDERING). See docs/ADR-052-THOTH-FOLD.md. |
| ADR-053 | **Accepted** — Extract Thoth as Standalone Package. Renumbered from ADR-016 (collision with TUI-PRIMARY-INTERFACE). See docs/ADR-053-THOTH-EXTRACTION.md. |
| ADR-054 | **Proposed** — Go owns the serving path; Python becomes a called extension. Renumbered from ADR-046 (collision with LOCAL-SOVEREIGNTY). See docs/ADR-054-GO-OWNS-THE-SERVING-PATH.md. |
