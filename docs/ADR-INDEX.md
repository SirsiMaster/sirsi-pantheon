...
**Total ADRs: 64 (+ ADR-031-A/B/C sub-decisions, + the ADR-054 companion contracts)** | **Next available: ADR-068**
...
| [ADR-051](ADR-051-ANUBIS-RA-SNE-SUPERVISOR-SPLIT.md) | Anubis/Ra Product Split — SNE Supervisor Configuration: Pantheon is the sole authority for SNE profile selection; Anubis→`interactive` (batch 4, 20 GB, yield), Ra→`fleet` (unconstrained); SNE stays neutral; `configs/supervisor/*.yaml` scaffold the binding | **Accepted** | 2026-08-03 |
| [ADR-052](ADR-052-A2A-CONDUIT-OPERATING-RULES.md) | A2A/Router-Conduit Operating Rules — Pantheon Adoption: seven A2A properties (durable authority, transactional events, edge-triggered marker, fenced spawn lease, adapter-bound identity, bounded consumer, evidence-bearing disposition); PR #430 = liveness input only; prohibited patterns (browser polling, recursive scheduling, cwd identity, second dispatch); task ledger is authoritative fleet projection; localhost:8734 is read-only SSE; resolves ADR-051 Pantheon/cross-repo numbering collision | **Accepted** | 2026-08-03 |
...
| [ADR-062](ADR-062-ROUTER-SERVICE-RA-CONCURRENCY.md) | Router as a Service — fleet concurrency for Anubis and Ra: `Store` interface + single `Resolve()`, SQLite (Anubis) and Postgres (Ra) backends, `sirsi router serve` over HTTPS with registered agent sessions, Cloud Run + Cloud SQL in `sirsi-nexus-live`, direct gcloud deploy | **Step 1 accepted and merged 2026-09-02; rev 4 amendments; steps 2–4 conditional on bind evidence** | 2026-09-02 |
| [ADR-065](ADR-065-ROUTER-OWNED-INFORMER-LANES-CARRY-NO-ARMING-LOGIC.md) | Router-Owned Watcher/Informer — Lanes Carry No Arming Logic: one informer per host subscribes for every registered agent over the store's existing edge signal (`ListenNotify`/`Wait`) and pushes to lanes; per-lane wake loops retired, the #636/#642 safety gates relocated intact (A29/Rule 0); every push acknowledged (`acked_at`, A2A property 6); arming strategy chosen from the lane's declared agent type in one router-side table; connection is the lane's contract | **Proposed** (owner decision 2026-09-13; SHA + SSA bind pending) | 2026-09-13 |
...
| ADR-047 | Unused — no file on disk. The earlier "(file exists — unindexed)" note was wrong. |
...
| ADR-051 | **Accepted** — Anubis/Ra Product Split: SNE Supervisor Configuration — Pantheon is the sole authority for which SNE profile (`interactive`/`fleet`) each product receives; SNE stays neutral; `configs/supervisor/anubis.yaml` + `ra.yaml` scaffold the binding |
| ADR-052 | **Accepted** — A2A/Router-Conduit Operating Rules: seven A2A properties (durable authority, transactional events, edge-triggered marker, fenced spawn lease, adapter-bound identity, bounded consumer, evidence-bearing disposition); prohibited patterns (browser polling, recursive scheduling, cwd identity, second dispatch); task ledger is authoritative fleet projection. |
...
| ADR-062 | **Step 1 merged; rev 4** — Router as a Service. SSA (`sirsi-software-admin`) accepted step-1 design authority (interface, resolver, direct-open gate) on 2026-09-02; service, migration and cutover need their own binds. Deploy is direct gcloud, never GitHub Actions (owner rule). |
| ADR-063 | **Accepted 2026-09-03 (Ra authority, owner mandate)** — Ra Charter, completion to completion. Standing mandate over the fabric layer: Ra may move to the next goal only after the current one reaches verified release, and only by a PR that enumerates every step of the next goal before any work starts; fixed owner gates on cost, custody, customer data, cut-over, bind authority; tokens are not progress. |
| ADR-064 | **Accepted 2026-09-10 (owner: keep both defaults)** — Isis AutoRenice governs runaways, not governed compute: pressure gate (default on), governed-compute allowlist, held renices inscribed, `sirsi report` surfaces demotions, `sirsi guard undo <pid>`. Provenance: the io-connect long-receipt collapse (`long-run-collapse-isis-watchdog-20260910.md`). |
| ADR-065 | **Proposed 2026-09-13 (owner decision; hardware-seat verdict ACCEPT 2026-09-16 w/ two conditions — rs-42 first, supervision covers token-host binding; SSA bind pending)** — Router-owned watcher/informer; lanes carry no arming logic. Same-day evidence for the decision: 7 of 9 M5 wake plists wrong (direct URL + plaintext token), a wrong `.zshenv` fallback on the same host, SSA 15 commits stale with the canon'd bundle handoff never operated, a SHA reply invisible to `ra`'s own pull beside a live second registry. One informer per host over the existing edge signal; push acknowledged as `acked_at` (A2A property 6, shipped narrowly first under SHA 20260913-071315 as `router acknowledge`); arming by declared agent type; connection is the lane's contract. Ledger rs-37. |
| ADR-066 | **Proposed 2026-09-16 (Ra decision; owner/SSA bind pending)** — Stack Lab wing authority: a wing record exists only on `origin/main` of its owning repo (`contracts/stacklab/<lane>-wing-v1.json`); `SirsiMaster/sirsi-stacklab` is the one registry, pinning each origin record by SHA-256 (anything unpinned under `wings/` is a draft); the router wing's `allowed_peer_wings` is the bidirectional roster; `sirsi stacklab doctor` makes stranding loud (`stranded/unbuilt`, `unpushed`, `unpinned`, `undeclared`, `invalid`) with CI + `--fix`. Provenance: io-connect wing committed on the M5 and never pushed (2026-09-16, PR #197). Follow-up rs-45 (roster: nexus-experience, finalwishes, apple-accelerator-routes). |
| ADR-067 | **Proposed 2026-09-25 (Ra decision; SSA review + owner bind pending)** — Credentialed machine-id adoption: identity is adopted as DATA by an authenticated act (the server-minted host token the session already holds), never inferred from string shape (the rejected bridge, which `TestThreadAuthorityIsHostScoped` proves unsafe). `AdoptTokenMachineID` / `sirsi thread adopt` binds a live host token to a stable machine id ONCE (one-way; exclusive — one machine id per live token, capping the worst case at a reversible DoS by an already-trusted fleet member, never silent impersonation); `HostIdentity` / `sameIdentity` resolve identities so a drifted hostname works after adoption and only after. Fixes the fabric-wide "thread authority: a session may only … on its own host" refusals. Schema v23 (SQLite + Postgres). Refs: rs-42/rs-43; `docs/router-service/IDENTITY_ARCHITECTURE.md` §4/§7; PR #785. |
| ADR-068 | Next available |

> **Last updated:** September 25, 2026 — indexed ADR-067 (Credentialed Machine-ID Adoption, proposed 2026-09-25; rs-42/rs-43; next available advanced to ADR-068). Earlier — September 16, 2026 — indexed ADR-066 (Stack Lab Wing Authority, proposed 2026-09-16; next available advanced to ADR-067); ADR-065 row records the hardware-seat ACCEPT verdict (claude-io acting as SHA, item 20260916-040736). Earlier — July 7, 2026 — indexed ADR-036 (Router v2 Durable Dispatch, accepted 2026-07-07; Phases 1-4 shipped #144/#164/#168/#174-era; cutover deliberately deferred to an owner-gated step); next available advanced to ADR-037. Earlier — July 4, 2026 — indexed ADR-035 (Runaway-Proof Execution, accepted 2026-07-04; canonizes the codex-APPROVED Phase-2 Dispatch Contract axioms at the architecture level and adds Sekhmet's independent host backstop — the "Runaway Executor" doctor finding + `sirsi router quarantine-worker`; provenance: the 2026-07-03/04 runaway-executor incident, case study `docs/case-studies/2026-07-04-runaway-executor.md`); next available advanced to ADR-036. Earlier — July 3, 2026 — indexed ADR-031-C (Broker Enforcement Must Be Universal, accepted 2026-07-03; a router-triage daemon and the warm-server's own LaunchAgent both bypassed the ADR-031-A/B broker by invoking `mlx_lm.*` directly — neither layer failed, neither was in the call path; both fixed and verified live; case study `docs/case-studies/2026-06-18-pantheon-did-not-prevent-oom.md` §6 updated same commit). Earlier — July 2, 2026 — indexed ADR-034 (Orchestration Brain, accepted 2026-07-02; governs PANTHEON_RULES A29; codifies + surfaces the existing `internal/router` wake substrate rather than rebuilding it); next available advanced to ADR-035. Earlier — July 1, 2026 — version-truth sweep: indexed ADR-033 (Remediation Catalog, accepted 2026-06-30, PR #125); next available advanced to ADR-034. Earlier — June 30, 2026: registry reconciled with disk (full-repo audit): added the previously-unregistered ADR-026 through ADR-032 (Horus ops-dashboard, router menubar, optional-SQLite, per-agent worktrees, native menubar popover, local-models-through-Pantheon + ADR-031-A/B resource governance, Mac-first roadmap); corrected the count and next-available pointer (ADR-032 was already taken on disk). Earlier milestone — ADR-023 **accepted**: one build-version contract (`internal/version`) replaces seven scattered `var version` literals; ldflags unified across all binaries; `internal/selfupdate` detects sibling (D2) and PATH (D3) drift locally with no network; `sirsi doctor` emits a `binary-drift` finding (`docs/ADR-023-BINARY-VERSION-CONTRACT.md`).
