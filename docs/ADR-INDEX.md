...
**Total ADRs: 55 (+ ADR-031-A/B/C sub-decisions, + the ADR-054 companion contracts)** | **Next available: ADR-056**
...
| [ADR-054](ADR-054-ONE-HORUS-UNIFIED-AGENT-FABRIC.md) | One Horus — the Unified Agent Fabric: one service identity over one durable store and one enforced lifecycle across four planes (identity & lifecycle, messages, work, models); REGISTER → BIND → WAKE → WORK → HEARTBEAT → EXIT/PARK → RECONSTITUTE → DEMIT; wake triggers are events, not timers. Companion contracts in [ADR-054-CONTRACTS](ADR-054-CONTRACTS-IDENTITY-AND-LEDGER-V7.md) (identity enforcement + ledger schema v7) share the 054 number by design | **Draft** | 2026-08-05 |
...
| ADR-054 | **Draft** — One Horus: the unified agent fabric. One service identity over one durable store and one enforced lifecycle across identity/lifecycle, messages, work, and models. Companion contracts (identity enforcement + ledger schema v7) share the 054 number by design, not by collision. |
| ADR-056 | Next available |
