...
**Total ADRs: 56 (incl. ADR-031-A/B/C sub-decisions)** | **Next available: ADR-057**
...
| ADR-047 | **Accepted** — Fold sirsi-thoth into sirsi-pantheon (single Go-native Thoth; retire standalone Node CLI; formerly mislabeled ADR-013) |
...
| ADR-052 | **Accepted** — A2A/Router-Conduit Operating Rules — Pantheon Adoption (seven A2A properties; prohibited patterns; task ledger as authoritative fleet projection; resolves ADR-051 numbering collision) |
| ADR-053 | Reserved — PR #450 (feat/horus-node-conduit): Horus as per-node conduit; router + observability = one flow |
| ADR-054 | Reserved — PR #451 (owner-gated): SNE Licensing / Anubis-Ra seam |
| ADR-055 | **Accepted** — Extract Thoth as Standalone Package (formerly mislabeled ADR-016; single source of truth for Go-native Thoth) |
| ADR-056 | **Proposed** — Go Owns the Serving Path; Python becomes a called extension (formerly mislabeled ADR-046; claude-nexus 2026-07-27) |
