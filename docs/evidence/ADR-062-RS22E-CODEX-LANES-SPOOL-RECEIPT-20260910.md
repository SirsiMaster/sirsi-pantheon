# ADR-062 step 20a.5 — four Codex lanes claim and close through the spool with token-clearing launch commands (2026-09-10T05:25Z–05:45Z, M5); codex-inference reads/updates after the 14:43Z fix
...
**codex-inference defect and fix.** Its 8 dispatches made zero relay calls and the loop logged `consumer made NO PROGRESS` with backoff. Cause: codex runs every command through a login shell, and `~/.zshenv` sourced `router-service.env` unconditionally, handing the https URL and the host token back to a lane that had chosen the spool and cleared the token; its sandbox has no DNS, so every router call failed. Fix: the source line runs only when `SIRSI_ROUTER_URL` is unset or empty — applied on both Macs by hand (reported manual action, this author); the candidate repair for already-cut-over hosts is PR #723 — head 051fcfde had an independently reproduced preservation failure (a filter I/O error was ignored and unrelated `.zshenv` content lost; SSA review 20260910-145230); head 47b6c852 corrects it (fail-fast filter, grep-error fixture) and is under review. Until it is bound and merged it is a pending candidate, separate from the reported manual state of the two hosts. Receipt inside the lane's own sandbox after the fix: `sirsi router pull codex-inference` → `72 open items for codex-inference: …`, `url=spool:///Users/thekryptodragon/.sirsi/relay tokenset=` (empty). Its loop was kickstarted at 14:43Z and the relay methods above followed. The same `--add-dir ~/.sirsi` shape is what made the other codex lanes' spool writes possible; why their early runs were not hit by the same override is not established (they may have started their first commands before re-sourcing), so a host-level fix is what protects all of them — currently the reported manual edit on both Macs, with PR #723 as the pending fixture-backed generator.
