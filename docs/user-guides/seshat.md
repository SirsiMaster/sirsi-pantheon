...
### Ingest knowledge
```bash
sirsi seshat ingest                           # All sources
sirsi seshat ingest --source chrome-history   # Chrome browsing history
sirsi seshat ingest --profile SirsiMaster     # Specific Chrome profile
sirsi seshat ingest --all-profiles            # All Chrome profiles
sirsi seshat ingest --since "7 days ago"      # Recent items only
```
...
sirsi seshat export thoth                     # Push to Thoth memory
sirsi seshat notebooklm                       # Export + open browser
sirsi seshat notebooklm --profile SirsiMaster # Specific profile
```

### List knowledge items
```bash
sirsi seshat list             # Show all ingested items with sources
```

### Manage adapters and profiles
```bash
sirsi seshat adapters         # List available source/target adapters
sirsi seshat profiles chrome  # List Chrome profiles
sirsi seshat open chrome --profile SirsiMaster  # Launch Chrome profile
```
...
### MCP Context Server
```bash
sirsi seshat mcp              # Start Seshat's MCP context server
```
