...
# 2026-08-05: Codex Inference wake identity corrected
#   Removed legacy cli-spawn from the codex-inference agent registry. The
#   checked-in fallback is now explicit `none`; the exact durable Codex app task
#   registers separately with `--wake automation` and its verified host PID.
#   Added a registry regression test so future drift cannot silently respawn a
#   headless worker instead of waking the durable task.
