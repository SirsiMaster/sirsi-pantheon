...
## 2026-07-14 — ADR-040 Load-Bearing Process Guard (claude-home)

**Trigger**: a live "reclaim RAM" request. A 25.8 GB process (53% of a 48 GB Mac), #1 by RSS, shown only as a nameless `Python`; a prior session had labeled it "a stuck/leaked MLX process holding a model resident without serving" and was about to SIGKILL it. Verifying the FULL argv (truncated in every RSS view) showed `gemma-capped-server.py --model gemma-4-12B-it-8bit --port 11434` — the launchd-managed local model server, actively serving (/health OK, R state, ~19% CPU), holding the model resident on purpose (the `27204354048` arg = its 25.3 GB cap). Killing it reclaims nothing durable (launchd/KeepAlive respawns → model reload), severs the fleet's zero-token triage, and acts on a misdiagnosis.

**What shipped** (branch `feat/guard-loadbearing-protect`, additive per A15): (1) `getProcessListWith` now captures full argv (`ps …,command` not `,comm`) — the tool was structurally blind to identity; `Name` = basename of the exe token only (never a `/` inside `--model path/arg`). (2) `internal/guard/loadbearing.go` — `isLoadBearingWith(p,proc)` classifies a local model/inference server from argv (signature list + generic serve-port-AND-references-a-model heuristic; loopback NOT a decision input so a plain 127.0.0.1 dev server stays killable), fetching argv on demand (`ps -o command=`) for bare interpreters. Reason names the SIZING LEVER (lower cap / smaller model / evict-idle), never SIGKILL. (3) Gated at the single chokepoint `isProtectedProcessWith`, so BOTH `SlayWith` and the orphan path `KillTrueOrphans` are covered — the orphan path was WORSE here because a launchd daemon has PPID=1 and looks like a true orphan. `SlayResult.Protected` + dashboard `/api/guard/slay` surface the reason for the menubar.

**Why it matters**: generalizes Rule A5 (Hapi "MUST NOT kill GPU processes actively inferencing") from GPU/VRAM to RAM — the same instinct, previously absent from the RAM slayer. The lesson ("see the full identity before you kill; distinguish load-bearing infra from a leak; re-size, don't SIGKILL") is now MECHANICAL and test-enforced, not a thing an operator must remember.

**Verification**: gofmt/vet/`go build ./...`/`go test ./internal/guard/ ./internal/dashboard/` all green; PLUS a live in-package run against the real machine confirming the running `sirsi-gemma` server is SEEN with full identity and SPARED with the sizing-lever reason (temp test, removed after). Canon: ADR-040 (new) + ADR-INDEX (40 total, next 041) + CHANGELOG Unreleased + this journal + memory.yaml guard line.
