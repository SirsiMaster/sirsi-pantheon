...
for file in build-dmg.sh verify-pantheon-package-identity.sh render-pantheon-homebrew-cask.sh verify-pantheon-release-asset.sh create-pantheon-lifecycle-plan.sh validate-pantheon-lifecycle-receipt.sh; do
...
grep -Fq 'lifecycle_contract=create-pantheon-lifecycle-plan.sh_then_validate-pantheon-lifecycle-receipt.sh' <<<"$output"

rm "$repo/scripts/validate-pantheon-lifecycle-receipt.sh"
if (cd "$repo" && bash scripts/verify-pantheon-unsigned-release-source.sh --pretag v0.23.14-beta "$head_sha") >/dev/null 2>&1; then
  echo 'expected missing lifecycle validator to fail' >&2
  exit 1
fi

printf '#!/usr/bin/env bash\n' > "$repo/scripts/validate-pantheon-lifecycle-receipt.sh"
