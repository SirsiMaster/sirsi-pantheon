...
func newTestFacade(t *testing.T) *dispatch.Facade {
	...
	if err := os.WriteFile(filepath.Join(root, "agents.json"), []byte(`{
		"agents": {
...
func TestWait_FacadeSendWakesWaiter(t *testing.T) {
	...
	go func() {
		items, err := f.Wait(context.Background(), "claude-home", 5*time.Second)
		if err != nil {
			t.Errorf("Wait: %v", err)
		}
		woke <- len(items)
	...
	if _, err := f.Send("codex-pantheon", "claude-home", "wake up", "", "x"); err != nil {
		t.Fatal(err)
	}
	select {
	case n := <-woke:
