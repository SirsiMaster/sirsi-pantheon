...
// tell" into "unhealthy"):
//
//  1. Healthy  — runner online in GH and/or no live PID mismatch.
//  2. Wedged   — PID alive + GH=offline + GH Actions component=operational.
//     A genuine process wedge; remediation warranted.
//  3. Suppressed — PID alive + GH=offline + GH Actions component=degraded or
//     major_outage. The mismatch is expected: GitHub's control plane cannot
//     report runner status accurately during an incident. Paging here would fire
//     at the exact moment the fleet can do nothing about it.
//
// The distinguishing signal (githubstatus.com/api/v2/components.json) is cheap
// and remains reachable precisely when you need it — the "API Requests"
// component stayed operational through the 2026-08-06 major_outage even as
// the "Actions" component degraded.
//
...
// RunnerHealthState is the combined OS+API liveness verdict for one runner.
type RunnerHealthState int

const (
	// RunnerStateUnknown means not enough data to make a call (API unavailable).
	RunnerStateUnknown RunnerHealthState = iota
	// RunnerStateHealthy — runner status is consistent (online, or stopped with no PID).
	RunnerStateHealthy
	// RunnerStateWedged — PID alive + GH=offline + Actions component=operational.
	// This is a genuine wedge requiring remediation.
	RunnerStateWedged
	// RunnerStateSuppressed — PID alive + GH=offline + Actions component=degraded/outage.
	// Offline status is expected during an Actions incident; suppress the alarm.
...
type RunnerHealth struct {
	...
	GHBusy      bool
	// State is the three-valued verdict (Healthy / Wedged / Suppressed / Unknown).
	State RunnerHealthState
	// Wedged is true when State == RunnerStateWedged (kept for callers that
	// predated the three-state model; new callers should switch on State).
	Wedged bool
}
...
// actionsOperationalFn is the injectable GitHub Actions component status prober
// (Rule A16/A21). Returns true when the Actions component is fully operational,
// false when degraded or in outage. Fail-open: returns true on any error so a
// reachability failure does NOT suppress a genuine wedge alarm.
var (
...
func defaultActionsOperational() bool {
	...
	// Component not found in the response — treat as operational (fail-open).
	return true
}
...
//
//   - RunnerStateWedged: PID alive + GH=offline + Actions=operational → alert.
//   - RunnerStateSuppressed: PID alive + GH=offline + Actions=degraded/outage
//     → suppress; offline is expected during an Actions incident.
//   - RunnerStateHealthy: all consistent.
//   - RunnerStateUnknown: GH API unavailable; no verdict possible.
//
...
func CheckRunnerHealth() []RunnerHealth {
	...
	// Query Actions component status only when the GH API is available and we
	// actually see offline runners (avoid a gratuitous HTTP round-trip on a
	// healthy fleet). We call it once and share the result across all runners.
	// actionsOK is set lazily below (first offline runner triggers the fetch).
	...
	for _, label := range labels {
		...
		case pid > 0 && gh.Status == "offline":
			...
			if actionsOK {
				...
				// ponytail: single fetch shared across all runners in one pass
...
//     not a genuine wedge, suppressed.
...
func ProbeRunnerWedge() Finding {
	...
	f.Body = fmt.Sprintf(
		"The liveness watch detected %d self-hosted runner(s) in wedge state: "+
			...
			"Note: if you see this during a GitHub Actions outage, the probe may have fired before "+
			"the outage was detected. Check githubstatus.com/api/v2/components.json.\n\n"+
