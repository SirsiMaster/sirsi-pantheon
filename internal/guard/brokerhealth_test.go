...
// TestApplyBrokerTruth_ScopedToBrokerPID is the R6 regression guard
// (PRD SNE_HETEROGENEOUS_COMPUTE.md): applyBrokerTruth overrides footprint for
// the broker PID ONLY, using the real Metal allocation — every other process's
// footprint passes through untouched (Rule A35 — scoped to the one PID the
// claim is actually about). brokerPID int replaces the old loadBearing map so
// the worker PID (also load-bearing) cannot accidentally receive broker numbers.
func TestApplyBrokerTruth_ScopedToBrokerPID(t *testing.T) {
	...
	const brokerPID = 4242

	// The broker PID: footprint/peak are overridden with the /health numbers.
	fp, peak := applyBrokerTruth(brokerPID, brokerPID, measuredFootprintGB, measuredFootprintGB)
	...
	// A different, non-broker PID: passes through unchanged, even though the
	// same (mocked) /health endpoint is reachable.
	otherFP, otherPeak := applyBrokerTruth(brokerPID, 9999, 500, 600)
	if otherFP != 500 || otherPeak != 600 {
		t.Errorf("non-broker PID footprint/peak = %d/%d, want unchanged 500/600", otherFP, otherPeak)
	}
}

// TestApplyBrokerTruth_WorkerPIDNotOverridden is the defect guard for the
// scoping bug claude-home identified in PR #625: when both broker and worker
// PIDs are alive, applyBrokerTruth must NOT apply the broker's mlx_active_bytes
// to the worker PID — the worker would appear as a ~27 GB hog in
// vitals/menubar/dashboard. Verified to fail before BrokerPID() was introduced.
func TestApplyBrokerTruth_WorkerPIDNotOverridden(t *testing.T) {
	old := getFetchBrokerHealthFn()
	t.Cleanup(func() { setFetchBrokerHealthFn(old) })

	const brokerMLX = int64(31_400_000_000) // 31.4 GB broker Metal allocation
	setFetchBrokerHealthFn(func() (BrokerHealth, error) {
		return BrokerHealth{MLXActiveBytes: brokerMLX}, nil
	})

	const brokerPID = 4242
	const workerPID = 5555
	const workerFootprint = int64(200_000_000) // 200 MB — the worker's real footprint

	// The worker must not receive the broker's 31.4 GB number.
	fp, _ := applyBrokerTruth(brokerPID, workerPID, workerFootprint, workerFootprint)
	if fp != workerFootprint {
		t.Errorf("worker footprint = %d, want unchanged %d — worker must not receive broker's mlx_active_bytes %d",
			fp, workerFootprint, brokerMLX)
	}
}
...
func TestApplyBrokerTruth_FailsSafeWhenUnreachable(t *testing.T) {
	...
	const brokerPID = 4242
	fp, peak := applyBrokerTruth(brokerPID, brokerPID, 4_200_000_000, 5_000_000_000)
