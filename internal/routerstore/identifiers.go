...
func (s *Store) AllocateIdentifier(namespace, title, owner string) (Identifier, error) {
	...
	err = s.withTx(func(tx *sql.Tx) error {
		...
		if err := tx.QueryRow(
			...
			return fmt.Errorf("routerstore: allocate %s: read high-water: %w", ns, err)
		}
		...
		if _, err := tx.Exec(
			...
			return fmt.Errorf("routerstore: allocate %s-%03d: %w", ns, next, err)
		}
...
func (s *Store) ClaimIdentifierNumber(namespace string, number int, title, slug, owner string) (Identifier, error) {
	...
	err = s.withTx(func(tx *sql.Tx) error {
		...
		switch err := row.Scan(&existingOwner, &existingTitle, &existingStatus, &existingSlug, &claimedAt); {
		case err == nil:
		...
		case errors.Is(err, sql.ErrNoRows):
			...
			if _, err := tx.Exec(
				...
				return fmt.Errorf("routerstore: claim %s-%03d: %w", ns, number, err)
			}
			out = Identifier{Namespace: ns, Number: number, Slug: slug, Title: title, Owner: owner, Status: IdentifierPublished, ClaimedAt: now}
			return nil
		default:
			return fmt.Errorf("routerstore: claim %s-%03d: read: %w", ns, number, err)
		}
	})
	if err != nil {
		return Identifier{}, err
