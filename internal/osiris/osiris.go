...
func (cp *Checkpoint) assessRisk() {
	...
	case cp.TotalChanges >= 30 || hoursSinceCommit >= 2:
		...
		cp.Warning = fmt.Sprintf("⚠ CRITICAL: %d uncommitted files, %.0f min since last commit — commit now!",
			cp.TotalChanges, cp.TimeSinceCommit.Minutes())
	case cp.TotalChanges >= 16:
		cp.Risk = RiskHigh
		cp.Warning = fmt.Sprintf("%d uncommitted files — consider a checkpoint commit", cp.TotalChanges)
	case cp.TotalChanges >= 6:
		cp.Risk = RiskModerate
		cp.Warning = fmt.Sprintf("%d uncommitted files", cp.TotalChanges)
	default:
		cp.Risk = RiskLow
	}
}
