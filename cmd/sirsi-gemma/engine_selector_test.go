...
func TestSelectRunnerDoesNotSilentlySwitchEngines(t *testing.T) {
	...
	for _, cfg := range []Config{
		...
		if _, ok := selectRunner(cfg, true, logger).(*disabledRunner); !ok {
			t.Fatalf("selected engine %q did not fail closed without its endpoint", cfg.Engine)
		}
	}
	if _, ok := selectRunner(Config{Engine: "mlx", SNEURL: "http://sne.test/v1"}, true, logger).(*MLXRunner); !ok {
		t.Fatal("explicit MLX selection was overridden by a legacy SNE endpoint")
	}
}
