...
func init() {
	...
	routerTaskAddCmd.Flags().StringVar(&taskAddBlockedBy, "blocked-by", "", "Same-agent task id this waits on, or a short reason for an external block; either way the task is not runnable until cleared (--blocked-by \"\" needs no lease)")
	...
	routerTaskUpdateCmd.Flags().StringVar(&taskUpdateBlockedBy, "blocked-by", "", "Same-agent task id this waits on, or a short reason for an external block; either way the task is not runnable until cleared. Empty clears it and needs no lease")
