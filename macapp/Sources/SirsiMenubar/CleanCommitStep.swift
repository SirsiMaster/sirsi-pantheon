    ...
    var body: some View {
        VStack(spacing: 6) {
            if let freed = freedText {
                ...
                Text("Freed \(freed)").sirsiFont(15, weight: .semibold)
                ...
                HStack(spacing: 8) {
                    ...
                    Text("Moving to Trash…").sirsiFont(.caption).foregroundStyle(.secondary)
                }
            } else if checkedCount == 0 {
                Text("Nothing selected to clean.").sirsiFont(.caption).foregroundStyle(.secondary)
            } else {
                Button {
                    onClean()
                } label: {
