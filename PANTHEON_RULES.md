...
> Established July 13, 2026 by owner directive and superseded operationally by ADR-057. The failure was not “daemon versus no daemon”; it was treating process/session liveness as proof of work. Custodian: 𓁢 the Router.

*   **Rule (ADR-057)**: `ctr` remains an operator-triggered reconciliation verb, but it is not the enforcement boundary. The durable Go store owns runnable truth, leases, wake events, acknowledgments, and completion evidence. A restartable supervisor reconciles that state and may run resident or on demand; its process existence never proves work. A lane may park only when router items, actionable ledger tasks, and audited canon gaps are all empty.
